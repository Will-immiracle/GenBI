...
如果我们想简单快速的插入大批量数据，存储过程是个不错的选择，上面这个存储过程，是我向表chart_test中插入300万条数据，**耗时130秒**左右，这里建议：
...
模拟一个百万级别的chart_test表（插入表），我们在没有做索引的字段上做一下查询看看，在300W数据中查询一个用户id为1860000的用户创建的所有表，消耗 2.239S ，获取到一条id为1860001的数据。    
